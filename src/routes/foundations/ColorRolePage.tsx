...
const BRANCHES: { id: string; title: string; lead: string }[] = [
  ...
  },
  {
    id: 'status',
    title: '상태',
    lead: '시스템이 알리는 결과입니다. 디자이너가 고르는 색이 아니라 사건이 정하는 색이라는 점에서 강조와 다릅니다. 같은 버튼이라도 삭제라는 사건이 붙으면 \'destructive\'가 됩니다.',
  },
  {
    id: 'line',
    title: '선',
...
const DECISION = [
  '다른 색 위에 올라가는 글자나 아이콘인가 — 전경',
  '면과 면을 가르는 선이거나 포커스를 알리는 링인가 — 선',
  '시스템이 알리는 결과(성공·경고·안내·위험)를 뜻하는가 — 상태',
  '사용자가 다음에 할 일을 가리키는가 — 강조',
  '무언가를 담는 바닥인가 — 표면',
]
...
export function ColorRolePage() {
  ...
  /* 전경 쪽에서는 반대 방향으로도 찾는다 — 'primary-foreground'의 짝은 'primary'다 */
          ...
          <table className="w-full text-left text-sm">
            <thead>
              <tr className="text-muted-foreground text-2xs tracking-widest">
                ...
                <th scope="col" className="px-3 py-2 font-bold">전경</th>
                <th scope="col" className="px-3 py-2 font-bold">배경 HEX</th>
                <th scope="col" className="px-3 py-2 font-bold">전경 HEX</th>
              </tr>
            </thead>
            <tbody>
              {pairs.map((row) => {
