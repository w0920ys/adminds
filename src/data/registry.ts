    ...
    guidelines: [
      {
        ...
        body: '계열이 하나면 항목마다 다른 색(fill)을 데이터에 직접 적어 하나의 링에 나눠 그린다. 계열이 둘 이상이면 config의 각 시리즈가 겹겹이 쌓인 링이 된다 — 서로 다른 데이터 모양이니 섞어 쓰지 않는다. 단일 계열의 값은 percentMax(기본 100)를 만점으로 보는 백분율로 다룬다 — 값이 이미 0~100 백분율이 아니면 percentMax를 그 값의 만점으로 넘긴다.',
        do: [],
