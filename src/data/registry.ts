...
export const components: ComponentMeta[] = [
  ...
  },
  {
    id: 'item',
    name: 'Item',
    aliases: ['아이템', '목록 행', '리스트 아이템', 'list item', 'row'],
    category: 'data-display',
    status: 'stable',
    addedIn: 'v0.24.0',
    changedIn: 'v0.24.0',
    purpose:
      'media·title·description·actions 조합이 행마다 달라지는 목록 한 칸을 만든다. 열이 고정된 데이터는 Table을, 폼 입력을 보여야 하면 Field를 대신 쓴다.',
    anatomy: [
      {
        part: 'container',
        label: 'Container',
        note: 'variant가 배경·테두리를, size가 안쪽 여백과 항목 사이 간격을 정한다. hover는 기본으로 켜져 있다',
      },
      {
        part: 'media',
        label: 'Media',
        note: '아이콘·아바타·이미지 중 하나. variant로 모양이 갈린다(icon은 사각 배경, avatar는 원형, image는 둥근 사각)',
        optional: true,
      },
      { part: 'content', label: 'Content', note: 'Title과 Description을 세로로 쌓는다. 남는 폭을 모두 차지한다' },
      { part: 'actions', label: 'Actions', note: '버튼 등 상호작용 요소. 폭이 모자라도 줄어들지 않는다', optional: true },
    ],
    properties: [
      {
        name: 'variant',
        title: 'Variant',
        description: '행의 배경과 테두리를 정한다.',
        display: 'row',
        options: [
          { value: 'default', note: '배경도 테두리도 없다' },
          { value: 'outline', note: '테두리와 카드 배경. 홀로 두드러져야 하는 행' },
          { value: 'muted', note: '옅은 배경. 부차적인 내용' },
        ],
      },
      {
        name: 'size',
        title: 'Size',
        description: '안쪽 여백과 항목 사이 간격을 정한다.',
        display: 'row',
        options: [
          { value: 'xs', note: '가장 빽빽하다' },
          { value: 'sm', note: '표 위 도구줄처럼 밀도가 높은 자리' },
          { value: 'default', note: '기본' },
        ],
      },
    ],
    guidelines: [
      {
        id: 'item-vs-field',
        title: 'Item과 Field를 구분한다',
        body: 'Field는 체크박스·인풋·라디오·셀렉트 같은 입력을 보입니다. 제목·설명·동작만 보이면 Item으로 충분합니다.',
        do: ['입력이 필요하면 Field를, 내용만 보이면 Item을 쓴다'],
        dont: ['입력 컨트롤을 Item 안에 직접 욱여넣지 않는다'],
      },
      {
        id: 'separator-between-items',
        title: '구분선은 손으로 넣는다',
        body: 'ItemGroup은 세로로 쌓기만 할 뿐 항목 사이에 선을 스스로 긋지 않습니다.',
        do: ['항목 사이에 선이 필요하면 ItemSeparator를 직접 넣는다'],
        dont: ['ItemGroup이 알아서 선을 그어 줄 거라 기대하지 않는다'],
      },
      {
        id: 'static-row-cancels-hover',
        title: '눌러도 반응 없는 행은 hover를 끈다',
        body: 'Item은 기본으로 hover 배경이 켜져 있습니다. 누를 수 없는 순수 표시용 행에 그대로 두면 누를 수 있다고 착각합니다.',
        do: ["동작이 없는 Item에는 className='hover:bg-transparent'를 준다"],
        dont: ['눌러도 아무 일도 없는 행에 hover 배경을 그대로 남기지 않는다'],
      },
      {
        id: 'size-matches-density',
        title: '밀도에 맞는 size를 쓴다',
        body: '한 화면에 많은 행을 보여야 하면 여백부터 줄입니다.',
        do: ['표 위 도구줄·좁은 사이드바는 xs·sm을, 일반 목록은 default를 쓴다'],
        dont: ['빽빽해야 하는 자리에 default 여백을 그대로 두지 않는다'],
      },
    ],
    usage: [
      { id: 'notification-list', title: '알림 목록', note: '아이콘 media와 제목·설명, 오른쪽에 시간을 나란히 둔다' },
      { id: 'settings-row', title: '설정 행', note: '제목·설명 옆에 Switch나 Button 같은 동작 하나를 둔다' },
      { id: 'search-result', title: '검색 결과', note: '아바타 media로 사람을, 오른쪽 동작으로 팔로우 같은 즉시 실행을 둔다' },
      { id: 'option-picker', title: '옵션 고르기', note: 'outline variant로 카드처럼 두드러지게 하고 고른 항목만 강조한다' },
    ],
    cases: [
      { id: 'no-media', title: '미디어가 없는 경우', note: 'ItemMedia 없이 Content만으로도 충분하다' },
      { id: 'title-only', title: '설명 없이 제목만 있는 경우', note: 'Description 없이 Title만 두어도 세로 정렬이 어긋나지 않는다' },
      { id: 'long-description', title: '설명이 긴 경우', note: '한 줄로 자르고 말줄임표로 나머지를 감춘다' },
      { id: 'narrow-screen', title: '좁은 화면', note: 'Content가 줄어들고 Actions는 줄어들지 않아 잘리지 않는다' },
    ],
    verified: false,
  },
  {
