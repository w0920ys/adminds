...
export const releases: Release[] = [
  {
    ...
    changes: [
      ...
      { target: 'Scroll Area · Command · Combobox · Date Picker', type: 'Fixed', note: '켜질 수 없는 클래스를 걷어냈어요. ScrollArea Viewport의 focus-visible 링은 Radix가 그 요소에 tabIndex를 주지 않고 ScrollArea도 prop을 흘려보내지 않아 절대 켜지지 않는데, 같은 회차에 실은 keyboard-focus-path 지침이 바로 그 사실을 적고 있었어요 — 코드와 지침이 서로 반대로 말하고 있던 셈이라 링을 걷고 왜 두지 않는지를 주석으로 남겼어요. Command 항목·Combobox 트리거·Date Picker 트리거의 cursor-not-allowed도 걷었어요. 같은 요소에 pointer-events-none이 함께 걸리면 히트 테스트를 받지 않아 커서 모양이 적용될 수 없어서, 나머지 컨트롤이 이미 쓰는 pointer-events-none + opacity-50만 남겼어요.' },
      { target: 'Registry', type: 'Updated', note: 'registry.json에 컴포넌트 여섯과 순수 함수 하나(command-filter)를 더했어요. Toggle은 toggle.tsx와 toggle-group.tsx 두 파일을 한 항목으로 실어요. adminds 묶음도 서른여덟 개를 전부 가리키게 갱신했어요. 구운 payload를 지키는 테스트도 넓혔어요 — 그전에는 files[].content만 견줘서, 파일이 없는 항목(adminds 묶음·tokens)은 payload가 낡아도 아무것도 잡지 못했거든요. 항목 메타까지 대조하게 했고, 손으로 적는 개수("서른여덟 개"·README의 "38개 전부")도 실제 컴포넌트 수와 견주게 했어요.' },
    ...
    reviewItems: [
      ...
      { label: '접근성 후속 묶음(포커스 링 대비, 이름 없는 라벨, Toast의 assertive 알림, ScrollArea Viewport에 tabIndex와 이름을 주어 순수 텍스트 상자에도 키보드 통로를 여는 것)을 언제 다룰 것인가', category: 'Components', completed: false },
