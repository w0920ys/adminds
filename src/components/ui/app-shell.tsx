...
export function AppShell({
  ...
  /**
   * 원본(momeokji-admin)엔 없던 확장 포인트 — 실제 화면에서는 항상
   * 비워 두고 md:h-svh를 그대로 쓴다. 이 문서 페이지의 Playground처럼
   * 셸을 페이지 안 한 상자에 가둬 미리 보여야 할 때만 md:h-[420px] 같은
   * 값으로 뷰포트 기준 높이를 덮어써 쓴다.
   */
  className?: string
}) {
  ...
  return (
    ...
    <div className={cn('bg-background text-foreground flex flex-col md:h-svh md:flex-row', className)}>
